...
public class MainActivity extends BridgeActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            // ÏßÑÎã® Î°úÍ∑∏ Ï∂úÎ†•
            android.util.Log.d("KakaoSDK", "=== KOE101 ÏßÑÎã® Î°úÍ∑∏ ?úÏûë ===");
            ...
            // KeyHash Î°úÍ∑∏ Ï∂úÎ†• (KOE101 ?îÎ≤ÑÍπÖÏö©)
            String keyHash = getKeyHash();
            android.util.Log.d("KakaoSDK", "KeyHash=" + keyHash);
            android.util.Log.d("KakaoSDK", "=== KOE101 ÏßÑÎã® Î°úÍ∑∏ ??===");

        } catch (Exception e) {
            android.util.Log.e("KakaoSDK", "Failed to initialize Kakao SDK", e);
        }

        // WebView textZoom 100% Í∞ïÏ†ú ?§Ï†ï (?çÏä§???êÌîÑ Î∞©Ï?)
        applyTextZoomSafely();

        // ?ÅÌÉúÎ∞??âÏÉÅ ?§Ï†ï
        getWindow().setStatusBarColor(Color.parseColor("#1e3a8a"));

        // ?úÏä§??UI ?∏ÏÖã Ï≤òÎ¶¨ (?§Î≥¥???Ä?ëÏùÑ ?ÑÌï¥)
        WindowCompat.setDecorFitsSystemWindows(getWindow(), true);

        // ?§Î≥¥?úÍ? ?¨Îùº????WebViewÍ∞Ä Î¶¨ÏÇ¨?¥Ï¶à?òÎèÑÎ°?Ï≤òÎ¶¨
        View rootView = findViewById(android.R.id.content);
        ViewCompat.setOnApplyWindowInsetsListener(rootView, (view, windowInsets) -> {
            Insets insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars());
            Insets imeInsets = windowInsets.getInsets(WindowInsetsCompat.Type.ime());

            // ?ÅÌÉúÎ∞??§ÎπÑÍ≤åÏù¥?òÎ∞î ?∏ÏÖãÎß??ÅÏö© (IME ?∏ÏÖã?Ä ?úÏä§?úÏù¥ ?êÎèô Ï≤òÎ¶¨?òÎèÑÎ°?
            view.setPadding(insets.left, insets.top, insets.right, insets.bottom);

            // ?åÎπÑ?òÏ? ?äÍ≥† ?ÑÌåå?òÏó¨ WebViewÍ∞Ä IME ?∏ÏÖã??Î∞òÏùë?òÎèÑÎ°?
            return WindowInsetsCompat.CONSUMED;
        });

        // ?§Î°úÍ∞ÄÍ∏?Î≤ÑÌäº Ï≤òÎ¶¨ (Android 13+ ?∏Ìôò)
        getOnBackPressedDispatcher().addCallback(this, new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                if (getBridge() != null && getBridge().getWebView() != null) {
                    ...
                    if (url != null) {
                        // URL?êÏÑú ÏøºÎ¶¨ ?åÎùºÎØ∏ÌÑ∞?Ä ?¥Ïãú ?úÍ±∞
                        ...
                        if (isMainPage) {
                            // Î©îÏù∏ ?îÎ©¥?êÏÑú????Ï¢ÖÎ£å
                            finish();
                        } else {
                            // ?§Î•∏ ?îÎ©¥?êÏÑú??JavaScriptÎ°?index.htmlÎ°??¥Îèô (?∏ÏÖò ?†Ï?)
    ...
    public void onResume() {
        super.onResume();
        // ?îÎ©¥ Î≥µÍ? ?úÏóê??textZoom ?¨Ï†Å??
        applyTextZoomSafely();
    }

    /**
     * WebView textZoom 100% Í∞ïÏ†ú ?§Ï†ï (?çÏä§???êÌîÑ Î∞©Ï?)
     * WebView Ï§ÄÎπ??Ä?¥Î∞ç ?¥Ïäà Î∞©Ï?Î•??ÑÌï¥ post + ÏßÄ???¨Ïãú???¨Ìï®
     */
