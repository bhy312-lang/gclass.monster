...
BEGIN
  ...
  -- 각 슬롯별 정원 체크 및 업데이트 (모든 선택된 슬롯에 대해)
  FOR v_slot_id IN
  ...
  -- 상태 결정 (모든 슬롯이 확정되었는지 확인)
  IF v_confirmed_count = v_total_slots THEN
    v_status := 'confirmed';
  ELSIF v_confirmed_count > 0 THEN
    v_status := 'waiting'; -- 부분 확정
  ELSE
    v_status := 'waiting'; -- 전체 대기
  END IF;
